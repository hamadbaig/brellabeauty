...
export async function GET() {
  try {
    await connectDB()
    let content = await HomepageContentModel.findOne({ collection: 'Brella Beauty' }).lean()
    
    if (!content) {
      // Create default content
      content = await HomepageContentModel.create({
        ...
        features: [
          ...
          },
          {
            id: 'cruelty',
            title: 'Cruelty Free',
            description: 'Never tested on animals, 100% ethical',
            icon: 'Heart',
            order: 2,
          },
          {
            ...
            description: 'Plant-based and eco-friendly ingredients',
            icon: 'Leaf',
            order: 3,
          },
          {
