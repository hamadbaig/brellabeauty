...
export function generateMetadata() {
    return {
        ...
        description: 'Browse our complete collection of luxury beauty products. Lip gloss, makeup, and skincare. Filter by category, price & availability.',
    }
}
...
export default async function ShopPage({ searchParams }: PageProps) {
    ...
    const pageTitle = category ?? subcategory ?? 'All Products'
            ...
            <main className="min-h-screen bg-pearl pt-20 sm:pt-24 pb-20">
                ...
                <div className="border-b border-mauve-200">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        <p className="label-blush mb-1">Our Collection</p>
                        <h1 className="font-serif text-3xl sm:text-4xl text-mauve-950">{pageTitle}</h1>
                    </div>
                </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                    <div className="flex gap-4 lg:gap-8 items-start">
                        ...
                        <Suspense fallback={<aside className="hidden lg:block w-64 h-64 bg-pearl-dark animate-pulse" />}>
                            <FilterSidebar
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between gap-4 mb-5">
                                <Suspense fallback={<div className="h-8 w-32 bg-pearl-dark animate-pulse" />}>
                            ...
                            {(category || subcategory || sp.minPrice || sp.maxPrice || inStock) && (
                                <div className="flex flex-wrap gap-2 mb-5">
                                    ...
                                    <a href="/shop" className="text-[10px] font-sans tracking-widest uppercase text-mauve-500 hover:text-blush transition-colors self-center ml-1">
                                        Clear all ×
                                    </a>
                                </div>
                            )}
                            ...
                            {result.products.length === 0 ? (
                                <div className="py-24 text-center border border-dashed border-mauve-200">
                                    <p className="font-serif text-2xl text-mauve-300 mb-3">No products found</p>
                                    <p className="text-sm font-sans text-mauve-400 mb-6">Try removing some filters to see more results.</p>
                                    <a href="/shop" className="btn-blush inline-block text-xs px-8 py-3">View All Products</a>
                                </div>
                            ) : (
                            ...
                            {result.total > 0 && (
                                <p className="text-center text-[11px] font-sans text-mauve-400 mt-4">
...
function ActiveChip({
    ...
    return (
        <a
            href={buildHref()}
            className="inline-flex items-center gap-1.5 text-xs font-sans px-2.5 py-1 bg-blush/10 text-blush-600 border border-blush/30 hover:bg-blush/20 transition-colors"
        >
            {label}
            <span className="text-blush/70 leading-none">&times;</span>
        </a>
    )
}
